    ...
    switch (view) {
      ...
      case View.AdminLogin:
        return (
          <AdminLogin
            apiUrl={API_URL}
            onSuccess={a => { setCurrentAdmin(a); setView(View.AdminDashboard); }}
          />
        );

      case View.AdminDashboard:
        if (!currentAdmin) { setView(View.AdminLogin); return null; }
        return (
          <AdminDashboard
            admin={currentAdmin}
            onLogout={() => { setCurrentAdmin(null); setView(View.Landing); }}
          />
        );
...
nSwitchToAdminLogin={() => setView(View.AdminLogin)}
  onSwitchToAnesthesiologistLogin={() => { setLoginError(''); setView(View.AnesthesiologistLogin); }}
  onSwitchToSurgeonLogin={() => { setLoginError(''); setView(View.SurgeonLogin); }}
  onSwitchToPatientView={() => { setLoginMessage(''); setView(View.PatientLogin); }}
            />
