...
import { Patient, Surgeon } from '../types';
import { AddPatientAndSurgicalConsentData } from '../App';
import { UserPlusIcon, ArrowRightOnRectangleIcon, CheckCircleIcon, ClockIcon, BriefcaseIcon, QrCodeIcon, BellAlertIcon } from './icons';
...
const surgicalSpecialties = [
    'Cirugía General', 'Cirugía Pediátrica', 'Cirugía Plástica', 'Ginecología y Obstetricia',
    'Neurocirugía', 'Oftalmología', 'Ortopedia y Traumatología', 'Otorrinolaringología', 'Urología'
];

const SurgeonDashboard: React.FC<SurgeonDashboardProps> = ({ surgeon, patients, onAddPatient, onLogout, isLoading, apiUrl }) => {
  const [formData, setFormData] = useState<Omit<AddPatientAndSurgicalConsentData, 'surgeryDateTime'>>({
      firstName: '', lastName: '', email: '', cedula: '', dateOfBirth: '', 
      sex: 'Masculino', consentPDFName: '', surgicalProcedure: ''
  });
  ...
  useEffect(() => {
    const fetchConsentForms = async () => {
        try {
            ...
            if (data.length > 0) {
                setFormData(prev => ({...prev, consentPDFName: data[0]}));
            }
        } catch (err) {
  ...
  const resetForm = useCallback(() => {
    setFormData({
        firstName: '', lastName: '', email: '', cedula: '', dateOfBirth: '', 
        sex: 'Masculino', consentPDFName: consentForms.length > 0 ? consentForms[0] : '', 
        surgicalProcedure: ''
    });
    setSurgeryDate('');
    setSurgeryTime('');
  ...
  const handleSubmit = useCallback((e: React.FormEvent) => {
    e.preventDefault();
    const { firstName, lastName, email, cedula, dateOfBirth, consentPDFName, surgicalProcedure } = formData;
    if (!firstName || !lastName || !email || !cedula || !dateOfBirth || !consentPDFName || !surgicalProcedure) {
  ...
  const patientPortalUrl = `${window.location.origin}#patient`;
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <fieldset className="space-y-4 pt-4">
                <legend className="text-md font-medium text-slate-800 -ml-1">Documento de Consentimiento</legend>
                 <div>
                    <label htmlFor="consentPDFName" className="block text-sm font-medium text-slate-700">Formulario PDF</label>
                    <select id="consentPDFName" value={formData.consentPDFName} onChange={handleFormChange} className="mt-1 block w-full pl-3 pr-10 py-2 border rounded-md" disabled={consentForms.length === 0} required>
                        {consentForms.map(form => <option key={form} value={form}>{form}</option>)}
                    </select>
                 </div>
              </fieldset>
              <button type="submit" className="w-full inline-flex justify-center items-center gap-2 px-4 py-2 border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-sky-600 hover:bg-sky-700">
                                ...
                                <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-3">
                                    ...
                                    <div className="flex items-center gap-2 flex-wrap self-end sm:self-center">
                                        <div className="flex gap-2 text-xs">
                                             ...
                                             <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full font-medium ${!patient.anesthesiaConsentPDFName ? 'bg-slate-100 text-slate-600' : patient.anesthesiaSignatureImage ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
        ...
        <Modal onClose={() => setShowQrModalFor(null)}>
            <div className="bg-white p-8 rounded-xl shadow-2xl text-center max-w-sm mx-auto">
                ...
                <div className="p-4 bg-white inline-block rounded-lg border">
                    <QRCodeCanvas value={patientPortalUrl} size={256} />
                </div>
