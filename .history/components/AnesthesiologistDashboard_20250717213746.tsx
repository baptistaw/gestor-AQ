...
import { Patient, Anesthesiologist } from '../types';
import { AddAnesthesiaConsentData } from '../App';
import { ArrowRightOnRectangleIcon, CheckCircleIcon, ClockIcon, DocumentPlusIcon, UserIcon, BellAlertIcon } from './icons';
...
const medicationOptions = [
    'AAS (Aspirina) - suspender 7 días antes',
    'Clopidogrel - suspender 5-7 días antes',
    'AINEs (Ibuprofeno, etc.) - suspender 3 días antes',
    'Anticoagulantes (Sintrom, Xarelto) - seguir pauta de hematología',
    'Metformina - no tomar la mañana de la cirugía',
    'Inhibidores SGLT2 (Jardiance, etc.) - suspender 3 días antes',
    'IECA/ARA II (Enalapril, etc.) - no tomar la mañana de la cirugía',
    'Diuréticos - no tomar la mañana de la cirugía',
    'Suplementos herbales (Ajo, Ginkgo) - suspender 1 semana antes',
    'Inhibidores de la MAO - consultar con especialista'
];
...
const AddAnesthesiaConsentForm: React.FC<{
  ...
  const [consentPDFName, setConsentPDFName] = useState('');
  const [consentForms, setConsentForms] = useState<string[]>([]);
   ...
   useEffect(() => {
    const fetchConsentForms = async () => {
        try {
            ...
            if (data.length > 0) setConsentPDFName(data[0]);
        } catch (err) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!consentPDFName || !instructions) {
        setError('El formulario de consentimiento y las instrucciones son obligatorios.');
        return;
    }
    onSave({
        consentPDFName,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 rounded-xl shadow-2xl max-w-lg w-full">
        <h2 className="text-xl font-bold mb-4">Añadir Consentimiento de Anestesia para {patient.firstName} {patient.lastName}</h2>
        {error && <p className="text-red-500 text-sm">{error}</p>}

        <div>
            <label htmlFor="consent-pdf-name" className="block text-sm font-medium text-slate-700">Formulario de Consentimiento</label>
            <select id="consent-pdf-name" value={consentPDFName} onChange={e => setConsentPDFName(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-slate-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" required>
                {consentForms.map(form => <option key={form} value={form}>{form}</option>)}
            </select>
        </div>
        <div>
            <label htmlFor="instructions" className="block text-sm font-medium text-slate-700">Instrucciones Especiales</label>
            <textarea id="instructions" value={instructions} onChange={e => setInstructions(e.target.value)} rows={3} className="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm" placeholder="p. ej., Por favor lea con atención la sección 3..." required></textarea>
        </div>
         <fieldset>
         ...
         <fieldset>
            <legend className="block text-sm font-medium text-slate-700">Medicación a Suspender</legend>
            <div className="mt-2 space-y-2 max-h-40 overflow-y-auto border border-slate-200 rounded-md p-2">
                {medicationOptions.map((med) => (
                    <div key={med} className="relative flex items-start">
                        <div className="flex h-6 items-center"><input id={`med-${med}`} type="checkbox" value={med} checked={medicationToSuspend.includes(med)} onChange={e => setMedicationToSuspend(prev => e.target.checked ? [...prev, e.target.value] : prev.filter(m => m !== e.target.value))} className="h-4 w-4 rounded border-gray-300 text-indigo-600"/></div>
                        <div className="ml-3 text-sm"><label htmlFor={`med-${med}`} className="text-slate-700">{med}</label></div>
                    </div>
                ))}
            </div>
        </fieldset>
        <div className="flex justify-end gap-3 pt-4">
            ...
            <button type="submit" className="px-4 py-2 text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700">Guardar y Enviar</button>
        </div>
    </form>
  )
}
                    ...
                    {filteredPatients.map(patient => (
                        <div key={patient.id} className={`p-4 border border-slate-200 rounded-lg bg-slate-50/50 flex flex-col sm:flex-row justify-between items-start gap-4 transition-opacity ${patient.isArchived ? 'opacity-60' : ''}`}>
                            <div className="flex-1 flex gap-4 items-start">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="mt-2 flex gap-2 text-xs">
                                        ...
                                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full font-medium ${!patient.anesthesiaConsentPDFName ? 'bg-slate-100 text-slate-600' : patient.anesthesiaSignatureImage ? 'bg-emerald-100 text-emerald-800' : 'bg-blue-100 text-blue-800'}`} title="Consentimiento de Anestesia">
                            ...
                            <div className="flex-shrink-0 self-end sm:self-center">
                                {patient.anesthesiaConsentPDFName ? (
