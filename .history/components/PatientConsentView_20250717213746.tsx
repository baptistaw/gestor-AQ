...
import { DocumentTextIcon, PencilSquareIcon, ArrowLeftOnRectangleIcon, ChatBubbleLeftRightIcon, ArrowDownTrayIcon, ClipboardDocumentListIcon, BriefcaseIcon, CalendarDaysIcon, BellAlertIcon, UserIcon, CakeIcon, UsersIcon, CheckCircleIcon } from './icons';
...
const ConsentSection: React.FC<{
  ...
  const consentPDFName = isSurgical ? patient.surgicalConsentPDFName : patient.anesthesiaConsentPDFName;
  ...
  if (!consentPDFName) return null; // Don't render if consent not added yet
  
  const consentPDFUrl = `${apiUrl}/uploads/${type}/${consentPDFName}`;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <aside className="lg:col-span-1 space-y-6">
            ...
            {(hasFastingInstructions || hasMedicationInstructions) && patient.anesthesiaConsentPDFName && (
              <div className="bg-white p-6 rounded-xl shadow-md">
                ...
                {surgeryDateTime && !patient.isArchived && (
                     <div className="bg-amber-50 border-l-4 border-amber-400 p-4 rounded-r-lg mb-4">
                        ...
                        <ul className="list-disc list-inside space-y-1 text-amber-700 mt-2 text-sm pl-2">
                           ...
                           {patient.medicationToSuspend.map((med, i) => <li key={i}><strong>{med.split(' - ')[0]}:</strong> Suspender antes del <span className="font-semibold">{calculateDeadline(surgeryDateTime, med)?.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' })}</span>.</li>)}
                        </ul>
                     </div>
                )}
              </div>
