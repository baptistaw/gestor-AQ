...
import { schedulePatientNotification } from './notify';
import { schedulePatientNotification } from './notify';
import { UpsertFastingPlanDTO, CreateSuspensionDTO } from './types';
...
/* --- 1. URL directa al portal paciente ---------------------------- */
const portalUrl =
  `${process.env.PUBLIC_BASE_URL || req.protocol + '://' + req.get('host')}` +
  `/#patient?email=${encodeURIComponent(patient.email)}`;

/* --- 2. E‑mail con pasos claros ----------------------------------- */
await transporter.sendMail({
  from: `"${process.env.EMAIL_FROM_NAME || 'Gestor de Consentimientos'}" <${process.env.EMAIL_FROM}>`,
  to: patient.email,
  subject: 'Acceso a sus Consentimientos y Próximos Pasos',
  html: `
    <div style="font-family:sans-serif;line-height:1.6;color:#0f172a">
      <h2 style="margin:0 0 0.5em">Hola&nbsp;${patient.firstName},</h2>

      <p>
        Adjuntamos los consentimientos informados que debes leer antes de
        tu cirugía.
      </p>
      ...
      <ol style="padding-left:1.2em">
        <li>
          Pulsa en este enlace seguro:<br>
          <a href="${portalUrl}" style="color:#2563eb">${portalUrl}</a>
        </li>
        <li>
          Tu <strong>usuario</strong> es tu correo:
          <em>${patient.email}</em><br>
          Tu <strong>contraseña</strong> es tu número de cédula.
        </li>
        <li>
          Al ingresar, elige <em>“Instalar aplicación”</em> para tener un
          acceso directo en la pantalla de inicio de tu móvil.
        </li>
      </ol>

      <h3 style="margin:1.5em 0 0.4em">¿Para qué sirve la plataforma?</h3>
      <ul style="padding-left:1.2em;list-style:disc">
        <li>
          Ingresa la <strong>fecha y hora exacta de tu cirugía</strong> cuando
          te la confirme tu cirujano.
        </li>
        <li>
          Activa las <strong>notificaciones</strong> para que te avisemos
          cuándo iniciar el ayuno y, si corresponde, cuándo suspender ciertos
          medicamentos.
        </li>
        <li>
          Podrás firmar electrónicamente los consentimientos que estén
          pendientes.
        </li>
      </ul>

      <p style="margin-top:1.5em">
        Ante cualquier duda, responde a este correo o comunícate con tu equipo
        médico.
      </p>
...
app.post('/api/ai/generate-medication-instructions', async (req, res) => {
  ...
  if (!medications || !surgicalProcedure) {
    return res.status(400).json({ message: 'Se requiere medicación y procedimiento.' });
  }
  ...
  try {
    ...
    if (!text) {
      return res.status(500).json({ message: 'La IA no devolvió texto.' });
    }
    res.json(JSON.parse(text));
  } catch (err) {
    console.error('AI error:', err);
    res.status(500).json({ message: 'Fallo al generar instrucciones.' });
  }
});
...
// ---------------------------------------------------------------------------
//  3.1  GET  /api/patients/:id/fasting-plan
// ---------------------------------------------------------------------------
...
+  } catch (err) {
+    console.error(err);
+    res.status(500).json({ message: 'Error al leer plan' });
+  }
+});

// ---------------------------------------------------------------------------
//  3.2  PUT  /api/patients/:id/fasting-plan   (crear / actualizar)
// ---------------------------------------------------------------------------
...
+    });
+
+    // 🔔 Programa alerta de ayuno (ejemplo a la hora de inicio)
...
// ---------------------------------------------------------------------------
//  3.3  GET  /api/patients/:id/suspensions
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
//  3.4  POST /api/patients/:id/suspensions
// ---------------------------------------------------------------------------
...
+  } catch (err) {
+    console.error(err);
+    res.status(500).json({ message: 'Error al crear suspensión' });
+  }
+});

// ---------------------------------------------------------------------------
//  3.5  DELETE /api/patients/:id/suspensions/:suspId
// ---------------------------------------------------------------------------
+app.delete(
+  '/api/patients/:id/suspensions/:suspId',
+  async (req, res) => {
+    try {
+      await prisma.suspension.delete({
+        where: { id: req.params.suspId },
+      });
+      res.sendStatus(204);
+    } catch (err) {
+      console.error(err);
