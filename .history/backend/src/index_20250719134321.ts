...
// --- Servir Frontend ---
// Esta sección debe venir DESPUÉS de todas las rutas de la API.
const frontendPath = path.join(__dirname, '..', '..');

// Servir archivos estáticos desde la raíz del proyecto.
// Se configura express.static para que resuelva automáticamente las extensiones de
// los módulos solicitados por el navegador y para que establezca un Content-Type
// personalizado para los archivos .ts y .tsx. Esto es crucial para que el
// transformador de Babel en el navegador funcione, ya que solicita módulos
// sin sus extensiones (p. ej., /App en lugar de /App.tsx).
app.use(express.static(frontendPath, {
  // Le dice a Express que busque estas extensiones si no encuentra una coincidencia exacta.
  // Por ejemplo, una solicitud para '/App' resolverá a '/App.tsx'.
  extensions: ['tsx', 'ts', 'js'],
  setHeaders: (res, filePath) => {
    const ext = path.extname(filePath);
    // Si el archivo es .ts o .tsx, lo servimos como javascript.
    if (ext === '.ts' || ext === '.tsx') {
      res.setHeader('Content-Type', 'application/javascript; charset=utf-t');
    }
  },
}));

// "Catch-all" para la SPA (Single Page Application).
// Si una solicitud no es una llamada a la API y no es un archivo estático
// servido por el middleware anterior, se sirve el archivo index.html principal.
// Esto permite que el enrutador de React se haga cargo de la navegación del lado del cliente.
