...
// Esta es la configuración correcta para servir archivos estáticos, incluyendo los
// archivos .ts/.tsx con el MIME type correcto para que Babel los pueda procesar.
// La mayoría de los archivos (css, js, images) serán servidos normalmente.
// Para los archivos que terminen en .ts o .tsx, se establece el header Content-Type.
app.use(express.static(frontendPath, {
    setHeaders: (res: Response, filePath) => {
        if (filePath.endsWith('.tsx') || filePath.endsWith('.ts')) {
            // Este es el MIME type que el navegador espera para un script de módulo.
            res.setHeader('Content-Type', 'application/javascript; charset=utf-8');
        }
    }
}));

// Esta ruta catch-all es para el enrutamiento de la Single Page Application (SPA).
// Si el navegador pide una ruta como '/patient-login', que no es un archivo físico
// en el servidor, este handler enviará 'index.html'. El router de React en el
// frontend se encargará entonces de mostrar el componente correcto.
// Debe ser la última ruta de la aplicación.
app.get('*', (req: Request, res: Response, next: NextFunction) => {
    // Evita que las rutas de API caigan aquí por error
