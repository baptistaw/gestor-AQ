...
// Stage 1: Create a specific route handler for .ts and .tsx files.
// This intercepts requests for these files, sets the correct MIME type for
// in-browser Babel, and then sends the file. This is more reliable than
// configuring express.static for non-standard browser file types.
app.get(/.+\.(ts|tsx)$/, (req: Request, res: Response, next: NextFunction) => {
    const filePath = path.join(frontendPath, req.path);
    
    // Set headers specifically for .ts/.tsx files to be treated as JavaScript modules.
    const options = {
        headers: {
    ...
    res.sendFile(filePath, options, (err) => {
        if (err) {
            // If the file is not found, pass the request to the next handler.
            // This allows the SPA catch-all to work if the path is not a real file.
            next();
        }
    });
});

// Stage 2: Serve all other static assets (like images, config.js, etc.) from the root directory.
app.use(express.static(frontendPath));

// Stage 3: The SPA Catch-all.
// If a request is not an API call and not a static file, serve the main
// index.html file. This allows the React router to take over. This must be the LAST route.
app.get('*', (req: Request, res: Response, next: NextFunction) => {
    // Prevent API routes from being caught here
    if (req.path.startsWith('/api/')) {
        return next();
    }
    // Don't serve index.html for module requests that might have fallen through
