...
const isEmailConfigured = process.env.EMAIL_HOST && process.env.EMAIL_USER && process.env.EMAIL_PASS && process.env.EMAIL_FROM;
...
if (isEmailConfigured) {
    ...
    }).catch((err) => console.error("Error al verificar la configuración del servidor de correo:", err));
} else {
     console.warn("ADVERTENCIA: Las variables de entorno para el envío de correo no están configuradas (EMAIL_HOST, EMAIL_USER, EMAIL_PASS, EMAIL_FROM). La funcionalidad de envío de correos estará deshabilitada.");
}
    ...
    try {
        const newPatient = await prisma.patient.create({
            data: {
                ...
                dateOfBirth: new Date(dateOfBirth), // Convert string to Date for Prisma
                sex,
...
// POST /api/patients/:patientId/send-consent-email
app.post('/api/patients/:patientId/send-consent-email', async (req: Request, res: Response) => {
    if (!transporter) {
        return res.status(503).json({ message: "La configuración de correo no está disponible en el servidor." });
    }
    ...
    try {
        ...
        if (!patient || !patient.email) {
            return res.status(404).json({ message: 'Paciente o email del paciente no encontrado.' });
        }

        const attachments: nodemailer.SendMailOptions['attachments'] = [];
        
        if (patient.surgicalConsentId) {
            const form = await prisma.consentForm.findUnique({ where: { id: patient.surgicalConsentId }});
            if (form) {
                attachments.push({
        ...
        if (patient.anesthesiaConsentId) {
            const form = await prisma.consentForm.findUnique({ where: { id: patient.anesthesiaConsentId }});
            if (form) {
                attachments.push({
                    filename: form.fileName,
                    content: form.fileContent,
        ...
        if (attachments.length === 0) {
            return res.status(400).json({ message: 'No hay consentimientos para enviar.' });
        }

        await transporter.sendMail({
            from: `"${process.env.EMAIL_FROM_NAME || 'Gestor de Consentimientos'}" <${process.env.EMAIL_FROM}>`,
            to: patient.email,
            subject: 'Sus Consentimientos Informados Preoperatorios',
            html: `
                <div style="font-family: sans-serif; line-height: 1.6;">
                    <h2>Hola ${patient.firstName},</h2>
                    <p>Adjunto encontrará sus documentos de consentimiento informado para su próximo procedimiento.</p>
                    <p>Por favor, revíselos cuidadosamente.</p>
                    <p>Si tiene alguna pregunta, no dude en contactar a su equipo médico.</p>
                    <br>
