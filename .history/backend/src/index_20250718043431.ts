...
app.get('/api/consent-forms', async (req: ExpressRequest, res: ExpressResponse) => {
...
app.get('/api/patients', async (req: ExpressRequest, res: ExpressResponse) => {
    try {
...
app.post('/api/patients', async (req: ExpressRequest, res: ExpressResponse) => {
    try {
        const { 
...
app.put('/api/patients/:id/anesthesia-consent', async (req: ExpressRequest, res: ExpressResponse) => {
    try {
        const { id } = req.params;
        const {
...
app.post('/api/surgeons/login', (req: ExpressRequest, res: ExpressResponse) => {
...
app.post('/api/anesthesiologists/login', (req: ExpressRequest, res: ExpressResponse) => {
    const { professionalLicenseNumber, password } = req.body;
...
app.put('/api/patients/:id/sign-surgical', async (req: ExpressRequest, res: ExpressResponse) => {
    try {
        const updatedPatient = await prisma.patient.update({
            where: { id: req.params.id },
            data: {
              son({ message: 'Credenciales inválidas o paciente no encontrado.' });
        }
    ...
    } catch  surgicalSignatureImage: req.body.signatureImage,
...
app.put('/api/patients/:id/sign-anesthesia', async (req: ExpressRequest, res: ExpressResponse) => {
    try {
        const updatedPatient = await prisma.patient.update({
            where: { id: req.params.id },
            data: {
