...
async function seedConsentForms(type: 'SURGICAL' | 'ANESTHESIA') {
 ...
 for (const fileName of pdfFiles) {
  const filePath = path.resolve(directoryPath, fileName);        // abs
  const relPath  = path.join(folder, fileName);                  // «surgical/…»

  // ⏳ tamaño del PDF (no lo cargamos en memoria)
  ...
  if (existing) {
    await prisma.consentForm.update({
      ...
      data:  { filePath: relPath, fileSize },
    });
  } else {
