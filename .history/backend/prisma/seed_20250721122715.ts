...
async function seedConsentForms(type: 'SURGICAL' | 'ANESTHESIA') {
  ...
  try {
    ...
    for (const fileName of pdfFiles) {
      const absPath     = path.resolve(directoryPath, fileName);      // absoluto
      const relPath     = path.relative(uploadsDir, absPath);         // «surgical/…»
      ...
      if (existing) {
        await prisma.consentForm.update({
          ...
          data:  { fileContent, filePath: relPath },                  // 👈 UPDATE
        });
      } else {
